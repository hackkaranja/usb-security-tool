...
async function loadSettings() {
  try {
    ...
    const yaraToggle = document.getElementById("yaraToggle");
    if (!autoScanToggle || !yaraToggle) return;
    ...
    autoScanToggle.checked = !!(s.scan_on_insert ?? s.auto_scan);
    yaraToggle.checked = !!(s.enable_yara ?? true);
  } catch (e) {
...
async function saveSettings() {
  ...
  const autoScanToggle = document.getElementById("autoScanToggle");
  const yaraToggle = document.getElementById("yaraToggle");
  if (!status || !autoScanToggle || !yaraToggle) return;
  status.textContent = "Saving...";
  status.style.color = "";

  try {
    const payload = {
      scan_on_insert: autoScanToggle.checked,
      enable_yara: yaraToggle.checked
    };
...
async function changePassword() {
  ...
  if (!strongPassword.test(newP)) {
    status.textContent = "Use 8+ chars with uppercase, lowercase, number, and symbol";
    status.style.color = "#dc3545";
    return;
  }
