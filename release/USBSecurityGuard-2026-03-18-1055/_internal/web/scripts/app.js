...
function addNewLog(logEntry) {
  // Push a new backend log entry into the in-memory list, then redraw the visible table.
...
async function refreshLogs(isManual = false) {
  try {
    ...
    // This is the main backend call that pulls log rows into the GUI.
...
function renderLogs(logs, markNewRows = false) {
  // This function is what turns log data into visible HTML rows in the log table.
  const tbody = document.querySelector("#logTable tbody");
  if (!tbody) return;
...
function startLiveLogRefresh() {
  ...
  // Poll backend every second so new log events become visible in the GUI automatically.
...
// =========================
//  QUARANTINE TAB - FILE MANAGEMENT
// =========================
